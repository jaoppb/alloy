//! The Vulkan rung of the cascade (`PRD-005:44`, `ADR-0009`).
//!
...
//! same three when `vulkano` lands, with nothing above this line changing.
//!
//! It is `Err(BackendUnavailable)` rather than `todo!()` — which the lint gate
//! denies anyway — because "no Vulkan on this machine" is an ordinary,
//! expected, recoverable condition, not an unfinished thought. **C-15** is F12.

use crate::application::ports::RenderBackend;
use crate::domain::error::GraphicsError;
use crate::domain::tier::BackendTier;

/// Attempts to bring up a Vulkan instance and device.

use crate::domain::ports::ColorResolver;
use crate::domain::property::Color;

/// Infrastructure adapter resolving CSS colors, supporting hex syntax and standard W3C named colors (C-15, C-16).
#[derive(Debug, Clone, Copy, Default)]
pub struct CssColorResolver;

impl CssColorResolver {
    /// Creates a new `CssColorResolver`.
    #[must_use]
    pub const fn new() -> Self {
        Self
    }

    /// Static convenience resolution method.
    #[must_use]
    pub fn resolve_static(raw: &str) -> Option<Color> {
        let trimmed = raw.trim().to_ascii_lowercase();
        if let Some(hex) = trimmed.strip_prefix('#') {
            return Self::parse_hex(hex);
        }

        Self::resolve_named(&trimmed)
    }

    /// Parses hexadecimal color notation `#RGB`, `#RGBA`, `#RRGGBB`, `#RRGGBBAA`.
    #[must_use]
    ...
    /// Resolves W3C CSS Color Level 4 named colors table.
    #[must_use]
    pub fn resolve_named(name: &str) -> Option<Color> {

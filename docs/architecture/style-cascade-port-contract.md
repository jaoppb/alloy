...
| 3    | Boundary aggregates: domain-owned, `#[non_exhaustive]`, schema version    | ✅ `DomSnapshot`, `StyledTree`, `LayoutBoxTree`, `ComputedStyle`, `StyledNode`, `LayoutBox` all domain-owned in `core/css`, `#[non_exhaustive]`; `css::PORT_SCHEMA_VERSION` is the single version knob. **`= 5`** — B4 reshaped `ComputedStyle` / `StyledNode` / `LayoutBox`, the fonts increment added `ComputedStyle::font_family`, and `5` widened `SUPPORTED_PROPERTIES` to accept the `background` / `border` shorthands (no new field; additive; `PRD-007` migration note below) |
...
| 7    | Frozen-API milestone                                                      | ✅ Frozen at `I3` (end of B4). `css::PORT_SCHEMA_VERSION = 5` is that surface (B4 froze it at `3`; the fonts increment bumped it to `4`; the "unstyled real sites" follow-up bumped it to `5`); any further boundary change bumps it and adds a `PRD-007` migration note                                                                                                                                                                                                               |

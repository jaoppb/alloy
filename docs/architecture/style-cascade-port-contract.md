...
| 3    | Boundary aggregates: domain-owned, `#[non_exhaustive]`, schema version    | ✅ `DomSnapshot`, `StyledTree`, `LayoutBoxTree`, `ComputedStyle`, `StyledNode`, `LayoutBox` all domain-owned in `core/css`, `#[non_exhaustive]`; `css::PORT_SCHEMA_VERSION` is the single version knob. **`= 7`** — B4 reshaped `ComputedStyle` / `StyledNode` / `LayoutBox`, the fonts increment added `ComputedStyle::font_family`, `5` widened `SUPPORTED_PROPERTIES` to accept the `background` / `border` shorthands, `6` added `PositionStyle`, `SizingConstraints`, `OverflowStyle` (49 properties), and `7` added `VisualStyle`, `TextAdvanceStyle`, `GridStyle`, `LogicalStyle`, expanding to 133 properties. |
...
| 7    | Frozen-API milestone                                                      | ✅ Frozen at `I3` (end of B4). `css::PORT_SCHEMA_VERSION = 7` is that surface (B4 froze it at `3`; fonts bumped to `4`; unstyled sites bumped to `5`; positioning/constraints/overflow bumped to `6`; visual/typography/grid/logical parallel tracks bumped to `7`); any further boundary change bumps it and adds a `PRD-007` migration note                                                                                                                                                                                                                                                                          |
...
`css::PORT_SCHEMA_VERSION` moved `2 → 3` in v0.5 B4, `3 → 4` in the v0.5 fonts increment, and `4 → 5` in the "unstyled
real sites" follow-up. Every change is **additive** — a new field, a new grouping, or (for `5`) a wider set of accepted
inputs for fields that already exist — never a removed or renarrowed one, so no existing `match` arm anywhere in the

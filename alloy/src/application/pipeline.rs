//! Headless render pipeline: parses HTML, computes styles, calculates layout,
//! paints to a display list, rasterizes via software CPU backend, and encodes to PNG.

use std::sync::Arc;

use css::{BlockLayout, CascadeResolver, LayoutEngine, UaCascade, ViewportConstraints};
use graphics::{
    Au, DisplayListBuilder, RenderBackend, SoftwareCpuBackend, SurfaceSize, SyntheticFontProvider,
};
...
pub fn render_html_to_png(html_str: &str, options: &RenderOptions) -> Result<Vec<u8>, AlloyError> {
    ...
    let mut backend = SoftwareCpuBackend::with_font_provider(font_provider);
